libname bias 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1';

**1-8-2014**
changed library names
added ethnic breakdown in a different way for sample characteristics for paper;
...
infile 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\explicit.txt' delimiter='09'x firstobs = 2;
input task_number 
question_number 
questionnaire_name $ 
question_name $ 
...
data bias.explicit; set bias.explicit;
repeat=0;
...
drop name IMPTASKTO 
Larts Lscience Tblack Twhite
admission age arts att
...
countrycit countryres customs edu employ ethnicityomb factordiscrimination
factorencouragement factorfamily factorinterest genident goal1 goal3
he1 he2 he3 he4 he5 he6 he7 he8 he9 he10
likeart likesci major nfc1 nfc2 nfc3 nfc4 nfc5 nfc6 nfc7 nfc8
...
raceomb rancollmen rancollwomen religionid reprbirth
...
science scompactc scompsatm scompsatv 
...
data bias.explicit; set bias.explicit; run;

proc freq data=bias.explicit; table citizen residence; run;
...
*import file;
data bias.sessions;
  infile 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\sessions.txt' DELIMITER='09'x LRECL=2000 FIRSTOBS  =  2;
  input session_id 
	user_id 
	study_name $ 
...
*save clean copy of dataset in external folder;
data bias.sessions; set bias.sessions; run;
...
INFILE 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\sessionTasks.txt' delimiter='09'x LRECL=2000 firstobs = 2;
INPUT	Session_ID 
Task_Number 
Task_ID $ 
Task_URL $ 
...
proc sort data = bias.sessiontasks; 
by session_id task_id; 

data bias.sessiontasks; set bias.sessiontasks; 
repeat=0;
...
*transpose file;
proc transpose data = bias.sessiontasks out=bias.sessiontasks; 
by session_id; 
id task_id; 
var task_number; run;
...
*keeping people who saw the reproduction questionnaire for the full sample. this technically eliminates people who saw the introduction page but not the questionnaire.;
...
proc freq data=bias.sessiontasks; table debriefed keep; run;
...
data bias.iat;
  ...
  infile 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\iat.txt' delimiter='09'x firstobs = 2;
  input block_number 
block_name $ 
block_trial_count 
...
data bias.reorder; set bias.temp;
...
data bias.iat; set bias.reorder; 
where task_name in ("demo_racea","demo_raceb","demo_racec","demo_raced"); 
drop block_name; run;

%iatCalc(bias, bias, iat, iat_clean, BLOCK, session_id, trial_latency, trialerror, 1, 2, 1); 

*create dataset in external folder with key variables;
data bias.iat_clean; set bias.iat_clean; 
...
proc sort data=bias.raw; 
by session_id BLOCK; run; 

*this session_id was problematic b/c had multiple repeated blocks and wasn't identified and deleted earlier;
data bias.raw; set bias.raw; 
if session_id = 16022944 then delete; run; 

data bias.iat_pairingdef; set bias.raw;
...
proc transpose data=bias.iat_pairingdef name=name out=bias.iat_pairingdef;
by session_id; 
var block_pairing_definition; 
id BLOCK; run;

*sort and merge iat_pairingdef and iat_clean and examine contents; 
proc sort data=bias.iat_pairingdef; 
by session_id; run; 

proc sort data=bias.iat_clean; 
by session_id; run; 

data bias.iat_combined; 
merge bias.iat_pairingdef bias.iat_clean; 
by session_id; run;

proc contents data=bias.iat_combined; run;
...
proc sort data=bias.sessions; 
by session_id; 

proc sort data=bias.explicit; 
by session_id; 

proc sort data=bias.iat_combined; 
by session_id; 
...
data bias.all; 
merge bias.sessions bias.explicit bias.iat_combined bias.sessionTasks; 
by session_id; run;

*examine contents and descriptives of each variable; 
proc contents data=bias.all; run;
proc means data=bias.all; run;
...
data bias.working; set bias.all;
format edu 1.;
...
data bias.working; set bias.working; 
...
proc freq data=bias.working; table race ethnicity; run;
...
proc freq data=bias.working; table race ethnicity racecomb; run;
...
ODS HTML BODY = 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\IAT Exclusions and Error Rates.html';

proc freq data=bias.working; 
...
proc freq data=bias.working; tables repstatus pregnow givenbirth; run;

proc freq data=bias.working; 
tables lastcycleday lastcyclemonth twocycleday twocyclemonth; run;

proc means data=bias.working; 
...
proc corr data=bias.working; 
var attitudeB4 attitude; run;

*save bias.working dataset externally and check it out;
data bias.cleaned; set bias.working; run;

proc means data=bias.cleaned; run;
proc contents data=bias.cleaned; run;

*shorten the dataset;
data bias.short; set bias.cleaned; 
...
data bias.short; set bias.short; 
if keep NE 1 then delete; run;

proc contents data=bias.short; run;
proc means data=bias.short; run;


TITLE 'Full Sample Characteristics - People who saw the reproduction questionnaire';
ODS HTML BODY = 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\Full Sample Characteristics - People who saw the reproduction questionnaire.html';

proc means data=bias.short; 
var age relid polid; run;

proc freq data=bias.short;
...
proc freq data=bias.short; table keep2 keep3 keep4 keep5 risk; run;

data bias.qualify; set bias.short; 
if keep2 = 0 then delete; run; 

data bias.qualify; set bias.qualify; 
...
proc freq data=bias.qualify;
table risk; run;

*creating standardized outcome variable;
proc standard data=bias.qualify MEAN=0 STD=1 OUT=bias.zqualify;
var IAT attitude warmth_black warmth_white risk whiteblack whitenonwhite; run;

proc contents data=bias.zqualify; run;
proc means data=bias.zqualify; run;

data bias.zqualify; set bias.zqualify; 
...
proc sort data=bias.qualify; by session_id; 
proc sort data=bias.zqualify; by session_id; 

data bias.qualified; 
merge bias.zqualify bias.qualify; 
by session_id; run;

proc contents data=bias.qualified; run;
proc means data=bias.qualified; run;

data bias.qualified; set bias.qualified; 
...
ODS HTML BODY = 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\Sample Characteristics for Qualifying Sample.html';

proc means data=bias.qualified; 
var age relid polid; run;

proc freq data=bias.qualified;
...
ODS HTML BODY = 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\Hypothesis Tests for Qualifying Sample.html';

proc corr data=bias.qualified; 
var risk IAT attitude warmth_black warmth_white att_composite; run;

TITLE 'Restricting to only White women';
data bias.white; set bias.qualified; 
if whiteblack NE 1 then delete; run;

proc corr data=bias.white; 
var risk IAT attitude warmth_black warmth_white att_composite; run;
...
ODS HTML BODY = 'C:\Users\chawkins\Documents\Research Projects\Race and Reproduction\analysis\study1\Follow up Tests for Qualifying Sample.html';

TITLE 'Controlling for Race - White or Black';
proc ttest data=bias.qualified; 
class whiteblack; 
var att_composite; run;

proc glm data=bias.qualified; 
model att_composite=whiteblack risk  / CLPARM; run;

proc reg data=bias.qualified; 
...
data bias.restricted; set bias.qualified; 
...
proc freq data=bias.restricted; 
table citizen age work; run;

proc glm data=bias.restricted; 
model att_composite=whiteblack risk  / CLPARM; run;

proc reg data=bias.restricted; 
model att_composite=whiteblack risk  / stb; run;
