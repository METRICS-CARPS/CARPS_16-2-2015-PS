...
d_raw_ <-read_excel('data/study1/Study_1_qualified.xls') 
```
...
d%<>%
  mutate(zIAT = scale(IAT, center = TRUE, scale = TRUE),
...
d%<>%
  mutate(zwarmth_diff = zwarmth_white - zwarmth_black)

#calculate composite score by averaging bias scores
d%<>%
  mutate(zatt_composite = (zIAT + zattitude + zwarmth_diff)/3)
```
